...
- [x] **가입 온보딩** (`/onboarding`, 신규): 회원가입 직후 벨트+그랄 → 주당 목표 수련 횟수+리마인더 시간 → 선호 스타일(선수 스타일 태그 재사용, 예시 선수 이름 표시) 3단계. 각 단계 건너뛰기 가능. `profiles` 테이블에 `weekly_goal`/`reminder_hour`/`preferred_style_tag`/`onboarding_completed` 컬럼 추가. 미들웨어에서 `onboarding_completed=false`인 로그인 사용자를 자동으로 이 화면으로 보냄(이메일 가입/구글 로그인 공통)
