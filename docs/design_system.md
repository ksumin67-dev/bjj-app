...
- [~] 홈 화면 (`HomeDashboard.tsx`) — 1차 적용: 헤더/주간 캘린더/오늘의 수련/벨트·XP/스트림 강도 섹션의 퍼플 그라디언트·스트림 무지개색·이모지를 브랜드 앰버 단일 액센트+lucide 아이콘으로 교체. "내가 배우고 싶은 기술"·"약점 스트림" 두 섹션은 아직 박스 카드+구팔레트 그대로라 다음 라운드에 이어서 적용
