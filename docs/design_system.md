...
- [~] 홈 화면 (`HomeDashboard.tsx`) — 1차: 색상 리뉴얼(퍼플→앰버, 무지개색→단일 액센트, 이모지→lucide). 2차: IA 정리 — 벨트/XP 섹션 + 스트림 강도 막대그래프 섹션(프로필 화면의 "벨트 여정"/"스트림 분포"와 완전 중복)을 통째로 제거하고, 벨트·XP·스트릭·최강스트림 한 줄 요약(탭하면 /profile 이동)으로 축소. "내가 배우고 싶은 기술"·"약점 스트림" 두 섹션은 홈에서만 제공되는 행동유도형 콘텐츠라 유지했지만 아직 구 박스 카드 스타일 그대로라 다음 라운드에 이어서 적용
