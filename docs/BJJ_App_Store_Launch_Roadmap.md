...
- [x] 스킬트리를 포지션 체계 → **선수 중심**으로 전면 재설계 (2026-09-14 결정·착수, 상세는 아래 참고)
...
**스킬트리 선수 중심 재설계 (2026-09-14)**: 기존 가드/탑/이스케이프/스탠딩 포지션 체계로 249개 기술 전부 커버하려던 방식은 기술 명명 불일치·유지보수 부담이 커서, 유명 선수의 대표 기술을 선수별로 묶는 방식으로 전환. Airtable에 Athletes 테이블(11명, ADCC/IBJJF 실제 커리어 성과 기반 "히어로 스탯" 포함) 신규 생성 + 기존 기술 68개(XG/SLX/FF/RDLR/KG/SG) 선수 태깅 완료. 진입 화면(스타일태그 탭+추천배너+선수리스트)과 선수 상세 페이지 코드 구현 완료(커밋 `3e26b5a`), `SkillTreeBrowser` 완전 교체. 아바타는 실사진 대신 실루엣 일러스트로 우선 출시(라이선스 이슈 회피), 나중에 확보되는 선수부터 순차 교체 예정. 남은 일: 로컬 push 후 실화면 확인, CG/MT/GP/DLR 등 나머지 기술 태깅, 프리미엄 게이팅(기본무료/심화유료) 적용. 자세한 내용은 Claude 메모리 `project_bjj_athlete_skilltree.md` 참고.
