...
- [ ] 디자인 시스템 전체 리뉴얼 (색상/타이포/컴포넌트 재정의 — `DESIGN_SYSTEM.md` 갱신 포함)
- [ ] 전 화면 UI 재작업 (홈/캘린더/스킬트리/시퀀스/프로필) — **2026-09-14 방식 전환**: 처음엔 `show_widget` 목업 승인 → 코드 직접 구현 방식(홈 화면 히어로카드/컷코너 스타일)으로 시도했으나, 목업과 실제 구현 결과물의 괴리가 반복되어 사용자가 원복 요청(커밋 `2c9954e`, 홈 화면은 리뉴얼 이전 원래 디자인으로 롤백됨). **이후 디자인 작업은 Claude 디자인(Design 아티팩트) 도구로 별도 진행 예정** — 완성된 디자인을 받은 뒤 코드에 반영하는 순서로 변경.
- [x] 스킬트리를 포지션 체계 → **선수 중심**으로 전면 재설계 (2026-09-14 결정·착수·완료, 상세는 아래 참고)
- [x] 주짓수 기술 DB(Airtable) 내용 정비 — 전수 감사 완료(2026-09-14), 결과는 매우 양호(빈 필드 3건만 발견해 수정). 상세는 Claude 메모리 `project_bjj_airtable_workflow.md` 7.5절 참고
- [x] **선수 시그니처 기술 학습 목표(찜) 기능 추가 (2026-09-15)**: "좋아하는 선수의 기술을 골라서 배우고 싶다"는 니즈 반영. Supabase `technique_goals` 테이블 신규(RLS 적용) + 선수 상세 페이지 하트 토글 + 홈 화면 "내가 배우고 싶은 기술" 카드 + 수련 기록 모달(TechPicker)에 목표 기술 고정 칩 섹션까지 한 번에 구현(커밋 `df4490e`). 찜 → 홈 노출 → 원클릭 수련기록까지 마찰 없이 이어지는 흐름이 핵심.
- [x] **스킬트리 메뉴 "선수/포지션" 이원화 (2026-09-15)**: 순수 선수 중심 브라우징의 사각지대 발견 — 전체 자식 기술 268개 중 113개(42%)만 선수 태깅되어 있어 나머지 58%는 브라우징 화면에 전혀 안 나오는 상태였음. "스킬트리"라는 이름 자체도 포지션 체계 도감을 뜻하는데 그 기능이 사라진 상태였음. 그렇다고 선수 스토리텔링(찜 기능의 기반)을 포기할 이유도 없어서, `/tree` 화면에 "선수"/"포지션" 탭을 두는 방식으로 절충(구 레벨링 Lv.1~4/소프트락 시스템은 부활 안 시킴, 예전 `SkillTreeBrowser.tsx`의 스트림→포지션카드 UI만 단순화해서 재활용). 새 라우트 `/tree/position/[positionId]` 추가 — 선수 상세와 동일한 플랫 리스트+찜 하트 UI 재사용. 커밋 예정.
- [ ] 그동안 발견된 기능 버그/UX 이슈 수정 (구체 목록은 착수 시 별도 정리)
- [ ] 리뉴얼 완료 후 Android 네이티브 셸 회귀 테스트 (로그인/리마인더/렌더링 재확인)

**스킬트리 선수 중심 재설계 (2026-09-14, 완료)**: 기존 가드/탑/이스케이프/스탠딩 포지션 체계로 249개 기술 전부 커버하려던 방식은 기술 명명 불일치·유지보수 부담이 커서, 유명 선수의 대표 기술을 선수별로 묶는 방식으로 전환. Airtable에 Athletes 테이블(11명, ADCC/IBJJF 실제 커리어 성과 기반 "히어로 스탯" 포함) 신규 생성 + 기존 기술 태깅(CG/MT/GP/DLR 포함 총 113개) + 선수 간 중복 해소용 신규 시그니처 기술 6개 추가. 진입 화면(스타일태그 탭+추천배너+선수리스트)과 선수 상세 페이지 코드 구현·배포 완료(커밋 `3e26b5a`~`17e3652`), `SkillTreeBrowser`와 구 포지션 드릴다운 레벨링 시스템(Lv.1~4, 난이도 그룹핑) 전부 제거. 아바타는 실사진 대신 실루엣 일러스트로 우선 출시(라이선스 이슈 회피), 나중에 확보되는 선수부터 순차 교체 예정. 프리미엄 게이팅은 기존 기술 상세 페이지의 `isPremium` 로직을 그대로 재사용해 별도 작업 없이 적용됨. 자세한 내용은 Claude 메모리 `project_bjj_athlete_skilltree.md` 참고.
